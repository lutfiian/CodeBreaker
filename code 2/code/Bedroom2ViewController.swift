...
class Bedroom2ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setupPaintInteraction()
        setupPopupElements()

        view.addSubview(character)
        character.frame = CGRect(x: self.view.center.x - (-280), y: self.view.center.y - 20, width: 100, height: 100)

    }
    
    override func viewDidAppear(_ animated: Bool) {
        animatePaintImageView()
    }
    ...
    private func setupPaintInteraction() {
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(handlePaintTapped))
        paintImageView.addGestureRecognizer(tapGesture)
        paintImageView.isUserInteractionEnabled = true
    }
    //manage clue and its attribute here
    @objc func handlePaintTapped() {
    ...
    private func animatePaintImageView() {
        ...
        UIView.animate(withDuration: duration,
                       ...
                       options: [.curveLinear],
                       animations: {
            self.paintImageView.transform = CGAffineTransform(rotationAngle: rotationAngle)
        }) { finished in
            UIView.animate(withDuration: duration,
                        delay: 0,
                        options: [.curveLinear],
            ...
            self.paintImageView.transform = CGAffineTransform.identity
            })
        }
    }
}
